...
function editor(lang, key, starter, challenge = false) {
  ...
  return `${localOnly ? csharpNotice() : ""}<div class="editor-shell" data-editor-key="${key}" data-lang="${lang}" data-challenge="${challenge}"><div class="editor-toolbar"><span>${icon("code", 16)} ${lang === "js" ? "practice.js" : "Program.cs"} <i class="editor-status">${lang === "js" ? "JavaScript" : (hosted ? "Local edition required" : ".NET " + (status.sdk || ""))}</i></span><div><button class="editor-reset" data-action="reset-code" title="Restore starter code">Reset</button><kbd>Ctrl ↵</kbd></div></div><div class="editor-area"><div class="line-numbers" aria-hidden="true">${draft
    .split("\n")
    .map((_, i) => i + 1)
    .join(
      "\n",
