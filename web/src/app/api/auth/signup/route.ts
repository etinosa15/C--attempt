...
// token + refresh token + current state exactly like login, and the client's reconcile
// step folds the device's local snapshot in. With confirmation ON there is no session
// yet: we return `{ pending: true }` so the studio can tell the learner to confirm
// their email and then sign in, instead of looking signed-in with a dead token. The
// device stays local until they confirm + log in. (See A#4: because signup logs in on
// success, existence is not fully hidden; bulk probing is bounded by the platform rate
// limiter. Accepted residual risk.)
...
export async function POST(request: Request) {
  ...
  // Confirmation pending: no session until the learner clicks the email link. Signal
  // that distinctly so the studio prompts them to confirm rather than pretending the
  // account is live (a token-less "signed in" state immediately 401s on the next call).
  if (!session)
    return withCors(request, NextResponse.json({ pending: true, email: data.user.email }));
